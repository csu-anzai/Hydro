...
int intersectionPolyhedronPlane(double** vEdgeStart, double** vEdgeEnd, int nEdges, double* pPlane, double* normalPlane, double** pIntersections)
{
    ...
    int maxInd = 0;
    double maxValue = abs(normalPlane[0]);
    for (int i = 1; i < 3; i++)
    {
        if(abs(normalPlane[i])>maxValue)
        {
            maxValue = normalPlane[i];
            maxInd = i;
        }
    }
