...
% set colors
\setbeamercolor{palette primary}{bg=FSUgarnet,fg=white}
\setbeamercolor{palette secondary}{bg=FSUgold,fg=white}
\setbeamercolor{palette tertiary}{bg=FSUgold,fg=white}
\setbeamercolor{palette quaternary}{bg=FSUgold,fg=white}
\setbeamercolor{structure}{fg=FSUgold} % itemize, enumerate, etc
\setbeamercolor{section in toc}{fg=FSUgold} % TOC sections

% Override palette coloring with secondary
\setbeamercolor{subsection in head/foot}{bg=FSUgarnet,fg=white}
...
\begin{frame}[shrink=10]{Multiresolution Representation}
	...
	\begin{equation*}
		h_{l} = \frac{b-a}{N_{l}}
	\end{equation*}
	The aim is to decompose the field at the finest (given) level of resolution into a representation at the
	coarsest level plus a series of differences at each finer level. The differences provide information about the
...
\begin{frame}[shrink=10]{Multiresolution Representation}
	...
	values at level $l$. An interpolating polynomial is used to preserve the cell averages.
	\begin{figure}
	    \centering
	    \begin{tikzpicture}[thick,scale=0.40, every node/.style={scale=0.6}]
